/*
LCM and HCF from Coding Challenges - Easy
by Silvio Duka
...
namespace LCMandHCF
{
    class Program
    {
        ...
        {
            Console.WriteLine($"The LCM of [{number1},{number2}] is {LCM()}");
            Console.WriteLine($"The HCF of [{number1},{number2}] is {HCF()}");
        }
        ...
        static int LCM()
        {
            ...
            long multipliedAll = number1 * number2;

            for (int i = number; i < multipliedAll + 1; i += number)
            {
                int t = i % number1 + i % number2;
